...
export default function TopBar({ title, onBack, rightIcon }: TopBarProps) {
  return (
    <div className="flex h-[56px] items-center justify-between px-5 bg-white">
      {onBack ? (
        <button onClick={onBack} className="shrink-0" aria-label="뒤로">
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#1B1B1B" strokeWidth={2}>
      ...
      {title && <p className="flex-1 text-center text-lg font-bold text-[#1B1B1B]">{title}</p>}
